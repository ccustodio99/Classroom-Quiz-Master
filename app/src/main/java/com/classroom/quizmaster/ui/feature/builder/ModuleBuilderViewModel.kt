...
class ModuleBuilderViewModel(private val container: AppContainer) : ViewModel() {
    ...
    fun onObjectivesChanged(value: String) {
        _uiState.value = _uiState.value.copy(objectives = value)
    }

    fun onSlidesChanged(value: String) {
        _uiState.value = _uiState.value.copy(slides = value)
    }

    fun onTimePerItemChanged(value: String) {
        _uiState.value = _uiState.value.copy(timePerItem = value)
    }
    ...
    fun save(onSuccess: () -> Unit) {
        viewModelScope.launch {
            ...
            val items = container.itemBankAgent.query(objectives, limit = 6)
            val module = Module(
